...
class PolitoWeb:
    ...
    def getListaMat(self):
        ...
        with requests.session() as s:
            ...
            hp = s.get('https://didattica.polito.it/portal/page/portal/home/Studente')
    ...
    def selectMat(self, id):
        ...
        with requests.session() as s:
            s.cookies = self.loginCookie
            selmat = s.get('https://didattica.polito.it/pls/portal30/sviluppo.chiama_materia?cod_ins='+self.listaMat[id][0]+'&incarico='+self.listaMat[id][1])
            #selmat = s.get('https://didattica.polito.it/pls/portal30/sviluppo.chiama_materia?cod_ins=06BQXOA&incarico=221082')
            self.matCookie = s.cookies
            self.getPathContent(self.workingFolder, '%2F')
    ...
    def getPathContent(self, cartella, path, code='0'):
        with requests.session() as s:
            ...
            json_result = s.get(self.baseUrl + 'action=list&path=' + path + ('' if code=='0' else ('&code=' + code)))

#            print(json_result.text)
            contenuto = json.loads(json_result.text)
#            print(contenuto)
    ...
    def downloadFile(self, cartella, name, path, code):
        with requests.session() as s:
            s.cookies = self.matCookie
            file = s.get(self.baseUrl + 'action=download&path=' + path + '%2F' + name + '&code=' + code, allow_redirects=True)
